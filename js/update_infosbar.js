function update_infosbar() {
    ...
    if (donnees.tr != undefined && donnees.state != undefined) {
        if (donnees.state >= 4 && donnees.laps_l == 1 && donnees.tr != -1 && donnees.tr != -2 && donnees.tr != -3 && donnees.tr != "unlimited" && donnees.styp == "Race" && donnees.laps != "unlimited") {
            ...
            //if (donnees.c in donnees.d && donnees.d[donnees.c].lc != undefined) {
            if (donnees.me_lc != undefined) {
                //timeremain = "<div style='font-size: 0.75em; vertical-align: top; top: 25%;'>" + "Lap " + (donnees.d[donnees.c].lc + 1) + "/" + donnees.laps + "</div>";
                timeremain = "<div style='font-size: 0.75em; vertical-align: top; top: 25%;'>" + "Lap " + (donnees.me_lc + 1) + "/" + donnees.laps + "</div>";
            } else {
                timeremain = "<div style='font-size: 0.75em; vertical-align: top; top: 25%;'>" + "Lap " + "--" + "/" + donnees.laps + "</div>";
            }
        } else {
    ...
    var tmp_isNaN = false;
    if (isNaN(donnees.windspeed) || isNaN(donnees.airtemp) || isNaN(donnees.tracktemp) || isNaN(donnees.winddir) || isNaN(donnees.humidity) || isNaN(donnees.fog) || isNaN(donnees.rain) || isNaN(donnees.track_wetness)) {
        tmp_isNaN = true;
    }

    if (!tmp_isNaN && (donnees.airtemp != undefined) && (donnees.tracktemp != undefined) && (donnees.winddir != undefined) && (donnees.humidity != undefined) && (donnees.fog != undefined) && (donnees.rain != undefined) && (donnees.track_wetness != undefined)) {
        ...
        tmp_sessioninfos += ", Rain " + "<span style='font-style: italic; font-weight: bold'>" + (donnees.rain * 100).toFixed(0) + "%</span>";

        tmp_sessioninfos += ", Wet " + "<span style='font-style: italic; font-weight: bold'>" + ((donnees.track_wetness * 100).toFixed(0) + "%") + "</span>";
    ...
    //if (donnees.sof != undefined) {
        ...
        if (nb_classes > 1) {
            ...
            for (var c = 1; c <= nb_classes; c++) {
                if (donnees.carclasscolor != undefined && (c in donnees.carclasscolor)) {
                    if (donnees.nbcars_class != undefined && c in donnees.nbcars_class && donnees.nbcars_class[c] > 0 && donnees.sof[c] != undefined) {
    ...
    */
    ...
    if (conso > 0) {
        ...
        // REM : depuis le 28/07/2023, je fais p - 1 pour les valeurs décimales car c'est plus logique, par exemple 0.9 veut dire qu'il faudra pitter
        ...
        if (p >= 1) {
            set_inner_html("fuelneed_h", "F2add " + text_fuelneed + " " + (Math.floor((p - 1) * 100) / 100) + " pits");
        } else {
            set_inner_html("fuelneed_h", "F2add " + text_fuelneed + " " + 0 + " pits");
        }
    ...
    if (banner_height != 0) {  // important car si c'est 0, ça rame
		...
		if (decalage > 120 || !donnees.is_iracing_started) {  // au-delà de 2 minutes de décalage on affiche OFFLINE
            set_inner_html("banner_live", "OFFLINE");
			$("#banner_live").css("background-color", "#dd0000");
		} else {
            if (donnees.live_status != undefined && donnees.live_status == 1) {  // on est à la fin du replay ou dans la voiture
                set_inner_html("banner_live", "LIVE");
                $("#banner_live").css("background-color", "#00dd00");
            } else {
                set_inner_html("banner_live", "REPLAY");
                $("#banner_live").css("background-color", "#ff8800");
            }
		}
