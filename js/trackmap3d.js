...
function trackmap() {
    ...
    //if (donnees.pexit != undefined && selected_idxjs == me && donnees.d[selected_idxjs].fr == 0) {
    if (donnees.pexit != undefined && donnees.d != undefined && me in donnees.d && donnees.d[me].fr == 0) {
        ...
        //if (selected_idxjs == me && donnees.d[selected_idxjs].fr == 0) {
            ...
            if (donnees.plost != undefined) {
                ...
                if (donnees.pexit > 0 && donnees.plost > 0 && trackmap_disp_predicted) {
                    //if (donnees.after_pit_plost != undefined && Math.abs(donnees.after_pit_plost - donnees.plost) <= 0.1) {  // à cause des arrondis, on regarde si la différence est supérieure à 1/10 s
                        //driver_on_trackmap("sphere_orange", "rgb(255,128,0)", 0.75, ldp_exit, me, 2.5, 1, 1);
                        driver_on_trackmap("sphere_orange", trackmap_predicted_color, 0.75, ldp_exit, me, 2.5, 1, 1);
                    //} else {
                    //    dp_exit = -1;
                    //}
                } else {
            ...
            if (k in track.x && k in track.y && donnees.k_max > 0) {
                ...
                if (plein != 0 && taille == 1 && classpos == 1 && trackmap_car_P1 && (trackmap_car_P1_myclass_only == 0 || is_myclass)) {
                    ...
                    if (objet["sphere_p1"][c] != undefined) {
                        ...
                        objet["sphere_p1"][c].material.dispose();  // pour éviter un leak memory
                    ...
                    if (objet["line_p1"][c] != undefined) {
                        ...
                        objet["line_p1"][c].material.dispose();  // pour éviter un leak memory
                ...
                if (opac < 1) {
                    objet[type][caridx].material.dispose();  // pour éviter un leak memory
                ...
                if (plein != 0 && taille == 1 && trackmap_disp_mode != 0) {
                    ...
                    if (trackmap_disp_mode == 1) {
                        ...
                        if (donnees.d[caridx].num != undefined) {
                            ...
                            if (objet["text_mode1"][caridx] != undefined) {  // si l'objet text est défini
                                ...
                                objet["text_mode1"][caridx].material.dispose();  // pour éviter un leak memory
                    ...
                    if (trackmap_disp_mode == 2) {
                        if (objet["text_mode2"][caridx] != undefined) {  // si l'objet text est défini
                            ...
                            objet["text_mode2"][caridx].material.dispose();  // pour éviter un leak memory
                        ...
                        if (classpos != undefined) {
                            if (c in objet["text_mode3"]) {
                                if (objet["text_mode3"][c][classpos] != undefined) {  // si l'objet text est défini
                                    ...
                                    objet["text_mode3"][c][classpos].material.dispose();  // pour éviter un leak memory
