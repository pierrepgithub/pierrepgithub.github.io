...
function change_turn_edit_ldp(event) {
    ...
    if (scrollup_turns_autoscroll_off && f3_box != 1 && (donnees.isontrack || !autoscroll_off_when_notincar) && event != undefined) {
...
function trackmap() {
    ...
    if (donnees.d != undefined) {
        ...
        //if (donnees.pexit != undefined && selected_idxjs == me && donnees.d[me].fr == 0) {
        if (donnees.pexit != undefined && donnees.d != undefined && me in donnees.d && donnees.d[me].fr == 0) {
            ...
            //if (selected_idxjs == me && donnees.d[me].fr == 0) {
            if (donnees.d[me].fr == 0) {
                if (donnees.plost != undefined) {
                    ...
                    // REM : j'ai enlevé la limite des 15 s car sur Interlagos par exemple on est à 9 s pour les pits
                    if (donnees.pexit > 0 && donnees.plost > 0 && trackmap_disp_predicted) {
                        //if (donnees.after_pit_plost != undefined && Math.abs(donnees.after_pit_plost - donnees.plost) <= 0.1) {  // à cause des arrondis, on regarde si la différence est supérieure à 1/10 s
                            //driver_on_trackmap("rgba(255,128,0,0.75)", ldp_exit, me, 2.5, 1, 1);
                            driver_on_trackmap(trackmap_predicted_color + "c0", ldp_exit, me, 2.5, 1, 1);
                            //console.log(ldp_exit)
                        //} else {
                        //    dp_exit = -1;
                        //}
                    } else {
        ...
        if ((!trackmap_car_myclass_only || is_myclass) && (!(donnees.d[caridx].pr) || (trackmap_car_inpits_disp == 1 || caridx == selected_idxjs))) {
            ...
            if (k in track.x && k in track.y && donnees.k_max > 0) {
                ...
                if (plein != 0 && taille == 1 && classpos == 1 && trackmap_car_P1 && (trackmap_car_P1_myclass_only == 0 || is_myclass)) {
                    ...
                    trackmap_context.beginPath(); //On démarre un nouveau tracé.
                    trackmap_context.strokeStyle = font_coul;
...
function draw_north(north) {
    if (trackmap_disp_north && (trackmap_circular_centered_on_driver == 0 || trackmap_circular == 0)) {
        if (north != 99) {  // si c'est 99 ça veut dire qu'il n'est pas défini
            north = north + angle - 1*Math.PI/2;  // REM j'ai remis le décalage Math.PI/2 car il est nécessaire à nouveau (depuis le 08/02/2023)
...
function draw_winddir(north, winddir) {
    ...
    if (north != 99 && trackmap_disp_wind && (trackmap_circular_centered_on_driver == 0 || trackmap_circular == 0)) {  // si c'est 99 ça veut dire qu'il n'est pas défini
        winddir = angle + north + winddir + Math.PI - 1*Math.PI/2;  // REM j'ai remis le décalage Math.PI/2 car il est nécessaire à nouveau (depuis le 08/02/2023)
        ...
        rayon = 0.5 * trackmap_winddir_coef * w / 30 * 1.5 * devicePixelRatio;
        ...
        x4 = x0 - dx * rayon * 0.5 + dy * rayon * 0.35;
        y4 = y0 - dy * rayon * 0.5 - dx * rayon * 0.35;*/
        // On prend une autre forme pour la flèche du vent pour ne pas confondre avec le Nord.
        var coord = {
            1: {'x': -0.25, 'y': -1},
            2: {'x': -0.25, 'y': 0.5},
            3: {'x': -0.5, 'y': 0.5},
        ...
        x1 = x0 - dx * rayon * coord[1].x - dy * rayon * coord[1].y;
        y1 = y0 - dy * rayon * coord[1].x + dx * rayon * coord[1].y;
        x2 = x0 - dx * rayon * coord[2].x - dy * rayon * coord[2].y;
        y2 = y0 - dy * rayon * coord[2].x + dx * rayon * coord[2].y;
        x3 = x0 - dx * rayon * coord[3].x - dy * rayon * coord[3].y;
        y3 = y0 - dy * rayon * coord[3].x + dx * rayon * coord[3].y;
        x4 = x0 - dx * rayon * coord[4].x - dy * rayon * coord[4].y;
        y4 = y0 - dy * rayon * coord[4].x + dx * rayon * coord[4].y;
        x5 = x0 - dx * rayon * coord[5].x - dy * rayon * coord[5].y;
        y5 = y0 - dy * rayon * coord[5].x + dx * rayon * coord[5].y;
        x6 = x0 - dx * rayon * coord[6].x - dy * rayon * coord[6].y;
        y6 = y0 - dy * rayon * coord[6].x + dx * rayon * coord[6].y;
        x7 = x0 - dx * rayon * coord[7].x - dy * rayon * coord[7].y;
        y7 = y0 - dy * rayon * coord[7].x + dx * rayon * coord[7].y;
        ...
        trackmap_context.lineTo(x2, y2);
        trackmap_context.lineTo(x3, y3);*/
        trackmap_context.moveTo(x1, y1);
        trackmap_context.lineTo(x2, y2);
        trackmap_context.lineTo(x3, y3);
        trackmap_context.lineTo(x4, y4);
        trackmap_context.lineTo(x5, y5);
        trackmap_context.lineTo(x6, y6);
        trackmap_context.lineTo(x7, y7);
        trackmap_context.fill(); //On trace seulement les lignes.
        trackmap_context.closePath();
    ...
    donnees.track_wetness = parseFloat(donnees.track_wetness);
    var tmp_isNaN = false;
    if (isNaN(donnees.windspeed) || isNaN(donnees.airtemp) || isNaN(donnees.tracktemp) || isNaN(donnees.winddir) || isNaN(donnees.humidity) || isNaN(donnees.fog) || isNaN(donnees.rain)) {
        tmp_isNaN = true;
    }
    ...
    if (!tmp_isNaN) {
        ...
        weather += ", Rain " + (donnees.rain * 100).toFixed(0) + "%";
        weather += ", Wet " + (donnees.track_wetness * 100).toFixed(0) + "%";
    }
