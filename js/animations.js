function animations() {
    ...
    if (donnees.d != undefined) {
        for (i in donnees.d) {
            ...
            if (Date.now() - click_status.start > focus_replay_delay && click_status.caridx == i && focus_replay_delay != 0) {  // si click long sur le pilote on affiche la ligne en rouge
            ...
            if (donnees.d[i].pr == 1) {
                ...
                tmp_col = '#000000';
                tmp_bg = '#ff9900';
                if (donnees.d[i].rt == donnees.d[i].st && donnees.d[i].st != 0) {  // cela signifie que c'est un tow suivi éventuellement d'une réparation ou d'un abandon
                    tmp_bg = '#ff0000';
                    tmp_col = '#ffffff';
                }
                ...
                if (show_name_orange_inpits) {
                    ...
                    if ((i == selected_idxjs && selected_driver_mode != 0 && fond_blanc != 3) || (donnees.d[i].fr == 1)) {
                        ...
                        if (relatives_after_pit == 1 && donnees.d[i].after_pit_dp != undefined && donnees.set_after_pit_nextpittimelost_to_0 == 0) {
                            dp = donnees.d[i].after_pit_dp;
                        } else {
                            dp = donnees.d[i].dp;
                        }
                        if (donnees.d[selected_idxjs] != undefined) {
                            if (relatives_after_pit == 1 && donnees.d[selected_idxjs].after_pit_dp != undefined && donnees.set_after_pit_nextpittimelost_to_0 == 0) {
            ...
            if (donnees.d[i].p2p_status != undefined) {
                if (donnees.d[i].p2p_status == 1 || donnees.d[i].p2p_status <= -2) {
