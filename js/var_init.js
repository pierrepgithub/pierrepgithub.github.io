...
function class_selected_rotation_fonction() {
    class_selected += 1;

    if (class_selected > donnees.nbclass) {
        if (class_selected_rotation_mode == 1) {
            class_selected = 0;
        } else {
            class_selected = 1;
        }
    ...
    // Comme c'est automatic, cela permet de ne pas afficher les changements de modes en bas à droite
    ...
    //console.log("classe rotation : ", class_selected);
}
...
function change_class_selected() {
    ...
    if (class_selected == 6 || class_selected == 7) {
        if (class_selected == 7) {
            class_selected_rotation_mode = 1;
        } else {
            class_selected_rotation_mode = 0;
        }
        class_selected = 1;
...
function init_var(create_canvas_trackmap) {
    ...
    // Permet de calculer le nombre de tours où il faut calculer le avg si on a mis avg_nblaps à 0
    avg1_nblaps_0 = 0
    avg2_nblaps_0 = 0
    avg3_nblaps_0 = 0
    ...
    menu_ = {
        ...
        "tv": {"state": 0, "mode": "down"},
        "pacing_line_leader": {"state": 0, "mode": "down"},
    };
...
modes_old = '';  // pour savoir si on a changé l'un des modes autoscroll ou f3_box (relatives)
modes_off = null;
